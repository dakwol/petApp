...
import { occupationList } from "../../../../../api/occupation/occupationList/occupationList";
import CheckBox from "@react-native-community/checkbox";
import UserProfessionItem from "./UserProfessionItem/UserProfessionItem";

const UserProfession:FC<IScreen> = ({navigation, route}) => {
    ...
    useEffect(()=>{
        setIsLoading(true)
        occupationList().then((resp)=>{
            if(resp.success){
                setIsLoading(false)
                let str = resp.data.occupations.map((item:any) => {
    ...
    useEffect(()=>{
        setArrInfo({...arrInfo, occupations: clickArray})
    }, [clickArray])


    const renderSpecial = ({item, index}:any) => {
       return (
        (item != undefined)?
            <>
                <UserProfessionItem 
    ...
    const switchScreen = () => {
        if(clickArray != ''){
            setErr(false)
            navigation.navigate('UserSpecialization', {infoUser: arrInfo})
        } else {
            setErr(true)
            errorMessage({
    ...
    return (
        ...
        >
                ...
                <View style={{marginBottom: 20}}>
                    <Text style={{alignSelf: "center", fontSize: 20, color: colors.greenPH, fontWeight: "bold", marginBottom: 20}}>{translate('registration.howUserProfession')}</Text>
